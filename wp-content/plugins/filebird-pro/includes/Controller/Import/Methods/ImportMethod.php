...
abstract class ImportMethod {
    ...
    public function run_import_folders( $folders, $attachments, $parent = 0 ) {
		...
		foreach ( $folders as $folder ) {
			...
			&& false !== $new_folder_id ) {
				FolderModel::setFoldersForPosts( $attachments[ $folder['term_id'] ], $new_folder_id );
			}

			if ( count( $folder['children'] ) > 0 ) {
				$new_child_folders = $this->run_import_folders( $folder['children'], $attachments, $new_folder_id );

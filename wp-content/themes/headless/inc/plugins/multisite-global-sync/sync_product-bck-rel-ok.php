            ...
            if (count($translations)) {
                foreach ($translations as $siteId => $postId) {
                    ...
                    if ($post != null) {
                        ...
                        // RELATIONSHIP FIELDS
                        // Check if $currentSiteReplProd is not empty
                        if ($siteId == 2 && $currentSiteReplProd) {
                            // Initialize an array to store translations for each relation
                            $stored_ReplProd_translations = [];

                            // Retrieve and store translations for each relation
                            foreach ($currentSiteReplProd as $key => $relation) {
                                $stored_ReplProd_translations[$key] = \Inpsyde\MultilingualPress\translationIds($relation->ID, 'Post', 1);
                            }
                            ...
                            foreach ($translations as $siteId => $postId) {
                                ...
                                if ($siteId != 1) {
                                    ...
                                    // Use the stored translations array for each relation
                                    foreach ($currentSiteReplProd as $key => $relation) {
                                        $relation->ID = isset($stored_ReplProd_translations[$key][$siteId]) ? $stored_ReplProd_translations[$key][$siteId] : '0';
                                    }

                                    // Update the 'replacement_product' field with the modified relations
                                    update_field('replacement_product', $currentSiteReplProd, $postId);

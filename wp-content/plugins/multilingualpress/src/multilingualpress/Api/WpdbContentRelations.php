...
final class WpdbContentRelations implements ContentRelations
{
    ...
    private function deleteRelationship(int $relationshipId): bool
    {
        ...
        if (!$this->relationshipMetaTable->exists()) {
            throw new NonexistentTable(__FUNCTION__, $this->relationshipMetaTable->name());
        }
        ...
        $deleteFromRelMetaTable = $this->wpdb->delete(
            $this->relationshipMetaTable->name(),
            [RelationshipMetaTable::COLUMN_RELATIONSHIP_ID => $relationshipId],
            '%d'
        );

        if ($deleteFromRelMetaTable === false) {
            return false;
        }
